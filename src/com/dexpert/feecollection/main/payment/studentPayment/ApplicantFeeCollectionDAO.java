...
public class ApplicantFeeCollectionDAO {
	...
	// get List Of Service Types if exists
	private List<ServiceTypeBean> getServiceTypeDetailList() {

		Session session = factory.openSession();
		try {
			Criteria criteria = session.createCriteria(ServiceTypeBean.class);
			List<ServiceTypeBean> list = criteria.list();
			return list;
		} finally {
			session.close();
			// TODO: handle exception
	...
	// get List Of faculty if exists
	private List<FacultyBean> getFacultyDetailList() {

		Session session = factory.openSession();
		try {
			Criteria criteria = session.createCriteria(FacultyBean.class);
			List<FacultyBean> list = criteria.list();
			return list;
		} finally {
			session.close();
			// TODO: handle exception
	...
	// get List Of faculty if exists
	private List<NationalityBean> getNationalityDetailList() {

		Session session = factory.openSession();
		try {
			Criteria criteria = session.createCriteria(NationalityBean.class);
			List<NationalityBean> list = criteria.list();
			return list;
		} finally {
			session.close();
			// TODO: handle exception
	...
	// get List Of faculty if exists
	private List<CourseBean> getCourseDetailList() {

		Session session = factory.openSession();
		try {
			Criteria criteria = session.createCriteria(CourseBean.class);
			List<CourseBean> list = criteria.list();
			return list;
		} finally {
			session.close();
			// TODO: handle exception
	...
	public Map<String, String> courseList() {
		...
		if (list.isEmpty()) {
			...
			for (Map.Entry<String, String> entry : map.entrySet()) {
				...
				System.out.println("courseBean" + entry.getKey() + "/" + "value is" + entry.getValue());
